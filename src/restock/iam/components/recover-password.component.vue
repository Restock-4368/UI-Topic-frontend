...
export default {
  ...
  methods: {
    recover() {
      if (this.email) {
        this.$toast.add({ severity: 'success', summary: 'Success', detail: `Recovery link sent to ${this.email}`, life: 3000 });
        this.email = "";
      } else {
        this.$toast.add({ severity: 'warn', summary: 'Warning', detail: 'Please enter your email.', life: 3000 });
      }
    }
  }
}
...
<template>
  <form @submit.prevent="recover" class="recover-password-form">
    <h2 class="title">Forgot your password?</h2>
    <p class="text-recover-password">
      Don't worry. Resetting your password is easy: just tell us which e-mail address you registered with.
    </p>
    <div class="input-field">
      <input type="email" v-model="email" placeholder="e-mail" required />
    </div>
    <button type="submit" class="btn solid">SEND</button>
    <button type="button" class="btn solid" @click="$emit('back')">Back</button>
  </form>
</template>
